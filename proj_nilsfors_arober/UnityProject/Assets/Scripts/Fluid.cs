...
public class Fluid : MonoBehaviour
{
    ...
    void ComputeDensity(Particle i)
    {
        ...
        i.density = 1; //density appears in denominators later on, if i has no neighbors this will get bad
        foreach (Particle j in i.neighbors)
        {
            i.density += Kernel.Poly6(i.position - j.position, h); 
        }
        i.density *= i.mass;
...
//            pressureGrad += j.mass * (i.pressure / Mathf.Pow(i.density, 2) + (j.pressure / Mathf.Pow(j.density, 2))) * Kernel.SpkiyGrad(i.position - j.position, h);
            pressureGrad += (i.pressure + j.pressure) / (2 * j.density) * Kernel.Poly6Grad(i.position - j.position, h);
        }
        pressureGrad *= i.mass;
...
//        pressureGrad *= i.mass;
    ...
    void ComputeViscosityForce(Particle i)
    {
        ...
        foreach (Particle j in i.neighbors)
        {
            viscosityForce += (j.velocity - i.velocity) / j.density * Kernel.ViscosityLap(i.position - j.position, h);
        }
